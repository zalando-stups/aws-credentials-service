...
def get_roles(username):
    ...
    response = requests.get(
        GROUPS_URL.format(uid=username),
...
def get_credentials(account_id: str, role_name: str, user: str, token_info: dict):
    ...
    if realm != "/employees":
        return connexion.problem(
            403, "Forbidden", "You are not authorized to use this service"
        )
    try:
        roles = get_roles(uid)
        ...
        return connexion.problem(
            500, "Server Error", "Failed to get groups: {}".format(e)
        )
    ...
    for account_role in roles:
        if (
            ...
            and account_id == account_role["account_id"]
        ):
            allowed = True
            break
    if not allowed:
        ...
        return connexion.problem(
            403, "Forbidden", "Access to requested AWS account/role was denied"
        )
